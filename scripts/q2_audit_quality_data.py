...
def main() -> int:
    ...
    W("The ratios are in the hundreds, not near 1. So the candidate form does")
    W("**not** reproduce B6/B7 to their storage precision either: it leaves a")
    W("median relative residual of order 1.7%, against a rounding quantum of")
    W("order 2e-05. Unlike the classic N-D law on the principal table, this is")
    W("*not* a generator recovery - the supplied quality mechanism is not the")
    W("`(Q^gamma * D)` effective-token form. The fitted gamma is a")
    W("best-approximation under a misspecified form, and is reported as such.")
    W("")
    ...
    W("   corroboration.")
    W("3. The candidate `(Q^gamma * D)` effective-token form does **not**")
    W("   reproduce B6/B7 to storage precision. gamma is therefore a")
    W("   best-approximation under a form the data do not support exactly, not")
    W("   a recovered generator constant.")
